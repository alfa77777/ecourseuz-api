...
from course.models import Course, CourseContent, CourseProgress, Review
from course.serializers import (
    ...
    ImportFileSerializer, CourseContentUploadSerializer,
)
from pagination import CustomPagination
from permissions import IsReviewAuthor, IsCourseAuthor
...
class CourseContentListView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        queryset = course.sections.order_by("position")
        serializer = CourseSectionSerializer(queryset, many=True, context={"request": request})
...
class CourseReviewListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return Review.objects.filter(course__slug=self.kwargs.get("slug")).order_by("-created_at")
...
class CourseContentUploadView(generics.CreateAPIView):
    queryset = CourseContent.objects.all()
    serializer_class = CourseContentUploadSerializer
    parser_classes = [MultiPartParser, FormParser]
    permission_classes = [IsCourseAuthor]

    def perform_create(self, serializer):
        course = Course.objects.get(pk=self.kwargs['course_id'])  # Получаем объект курса по его идентификатору
        serializer.save(course=course)  # Привязываем видео к курсу
